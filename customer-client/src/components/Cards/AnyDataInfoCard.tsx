        ...
        <div onClick={() => {onClick(reservation)}} className="bg-white overflow-hidden shadow rounded-lg cursor-pointer hover:shadow-lg transition-shadow">
            <div className="px-4 py-5 sm:p-6">
                <div className="flex items-center">
                    <div className={`flex-shrink-0 rounded-md p-3 ${reservation.isActive ? 'bg-blue-500' : (reservation.isDeleted || reservation.isCancelled) ? 'bg-red-500' : 'bg-green-500'}`}>
                    ...
                    <div className="ml-5 w-0 flex-1">
                        <dl>
                            <dt className="text-sm font-medium text-gray-500 truncate">
                                {reservation.Table.name} - {new Date(reservation.createdAt).toLocaleDateString()}
                            </dt>
                            <dd>
